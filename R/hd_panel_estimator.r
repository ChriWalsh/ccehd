...
#' @export
hd_panel_estimator <- function(data, obs_N, obs_T, TRUNC = 0.05, NFACTORS = NULL, variant = "LS", penalty_choice = NULL, NFOLDS=NULL, NLAMBDA=NULL, scree_plot = TRUE){
  ...
  if (variant=="LS"){
    ...
    results <- list(coefs=coef_est,K_hat = K_hat,Lambda = 0)
    ...
    if(penalty_choice == "None"){
      ...
      if(class(NLAMBDA) == "numeric"){
        if(is.naturalnumber(NLAMBDA)==TRUE){
          NLAMBDA <- NLAMBDA
          message(paste("User-supplied number of penalties given by 'NLAMBDA' = ",NLAMBDA," is used in call to glmnet."))

          fit_Lasso <- glmnet::glmnet(x=X_hat,y=Y_hat,family="gaussian",alpha=1,nlambda = NLAMBDA,intercept=FALSE)
          ...
          fit_Lasso <- glmnet::glmnet(x=X_hat,y=Y_hat,family="gaussian",alpha=1,intercept=FALSE)
    ...
    # Closes penalty == "None" block
      ...
      if(class(NFOLDS) == "numeric"){
        if(is.naturalnumber(NFOLDS)==TRUE){
          NFOLDS <- NFOLDS
          message(paste("User-supplied number of penalties given by 'NFOLDS' = ",NFOLDS," is used in call to cv.glmnet."))

          fit_Lasso.cv <- glmnet::cv.glmnet(x=X_hat,y=Y_hat,nfolds=NFOLDS,family="gaussian",alpha=1,intercept=FALSE)
        ...
        fit_Lasso.cv <- glmnet::cv.glmnet(x=X_hat,y=Y_hat,family="gaussian",alpha=1,intercept=FALSE)
